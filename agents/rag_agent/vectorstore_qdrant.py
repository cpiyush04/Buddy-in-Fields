...
class VectorStore:
    ...
    def load_vectorstore(self) -> Tuple[QdrantVectorStore, LocalFileStore]:
        ...
        if self._does_collection_exist():
            self.logger.warning(f"Collection {self.collection_name} exists. Deleting it to ensure correct embedding dimensions...")
            self.client.delete_collection(self.collection_name)
            
        # Now we always create a fresh collection with the correct config (384 dims)
    ...
    def add_documents(self, documents: List[Document]) -> List[str]:
        """
        Adds LangChain Document objects to Qdrant and LocalFileStore.
        """
        try:
            self.logger.info(f"Adding {len(documents)} new documents to vector store...")
            
            # 1. Generate IDs for new docs if missing
            doc_ids = []
            ...
            # 3. Add to Qdrant (Vectors)
            qdrant_vectorstore.add_documents(documents=documents, ids=doc_ids)

            # 4. Add to Local Docstore (Content)
            docstore = LocalFileStore(self.docstore_local_path)
            encoded_chunks = [doc.page_content.encode('utf-8') for doc in documents]
            docstore.mset(list(zip(doc_ids, encoded_chunks)))

            self.logger.info("Successfully added documents.")
            return doc_ids
