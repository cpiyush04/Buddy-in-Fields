...
import logging
from typing import List, Optional, Dict, Any
# <--- [NEW] Import Document to create memory objects
...
class MedicalRAG:
    ...
    def ingest_directory(self, directory_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not files:
                self.logger.warning(f"No files found in directory: {directory_path}")
                return {"success": True, "documents_ingested": 0, "chunks_processed": 0, "processing_time": time.time() - start_time}
    ...
    # --- [NEW] DYNAMIC LEARNING METHODS ---

    def ingest_new_document(self, file_path: str):
        """
        Wrapper for ingest_file called by app.py background tasks.
        Uses your existing full pipeline (parse -> summarize -> chunk -> store).
        """
        self.logger.info(f"🚀 [Dynamic Learning] Learning from new file: {file_path}")
        result = self.ingest_file(file_path)
        if result["success"]:
            self.logger.info("✅ File successfully internalized into Knowledge Base.")
        else:
            self.logger.error(f"❌ Failed to learn from file: {result.get('error')}")

    def add_memory(self, text: str):
        """
        Saves a conversation snippet (Q&A pair) as a memory into Qdrant.
        Called by app.py background tasks.
        """
        self.logger.info(f"🧠 [Dynamic Learning] Memorizing conversation snippet...")
        
        try:
            # Create a LangChain Document object
            # We tag it as 'episodic_memory' so we can potentially filter or weight it differently later
            doc = Document(
                page_content=text,
                metadata={
            ...
            # Use the new add_documents method in VectorStore
            # This works because we updated vectorstore_qdrant.py to support add_documents
            self.vector_store.add_documents([doc])
            self.logger.info("✅ Conversation snippet saved to memory.")
