...
from agents.image_analysis_agent import PlantDiseaseAgent, PestAgent, GeneralImageAgent
...
# These are lightweight until .analyze() is called

plant_agent = PlantDiseaseAgent()
pest_agent = PestAgent()
general_agent = GeneralImageAgent()
...
class AgentConfig:
    ...
    # Define valid nodes to prevent routing errors
    VALID_NODES = [
        "CONVERSATION_AGENT", 
        "RAG_AGENT", 
    ...
    # --- UPDATED AGRICULTURAL PROMPT ---
    DECISION_SYSTEM_PROMPT = """You are an intelligent agricultural triage system.
    Your job is to route the user's request to the best agent based on content and images.

    Available agents:
    1. CONVERSATION_AGENT - General chat, greetings, non-farming questions.
    2. RAG_AGENT - Specific farming knowledge (fertilizers, crops, soil health) from manuals.
    3. WEB_SEARCH_PROCESSOR_AGENT - Recent market prices, weather, news.
    4. PLANT_DISEASE_AGENT - Images of sick leaves, fruit, vegetables, spots, lesions.
    5. INSECT_AGENT - Images of bugs, worms, beetles, pests.
    6. GENERAL_AGENT - General farm images (tractors, tools, unknown objects) or general visual questions.

    Guidelines:
    - If NO image: Route to CONVERSATION, RAG, or WEB_SEARCH.
    - If IMAGE is present: MUST route to PLANT_DISEASE, INSECT, or GENERAL_AGENT.
    - "What is this bug?" -> INSECT_AGENT
    - "Why is my leaf yellow?" -> PLANT_DISEASE_AGENT
    - "What tool is this?" -> GENERAL_AGENT
...
def create_agent_graph():
    ...
    def analyze_input(state: AgentState) -> AgentState:
        ...
        if isinstance(current_input, dict) and "image" in current_input and current_input["image"]:
            has_image = True
            print("IMAGE DETECTED IN INPUT")
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        try:
            ...
            print(f"[ROUTER ERROR] Decision parsing failed: {e}")
            decision = {"agent": "CONVERSATION_AGENT", "confidence": 1.0}

        # 🔒 SANITIZE AGENT NAME (CRITICAL)
        agent = decision.get("agent", "CONVERSATION_AGENT")

        if agent not in AgentConfig.VALID_NODES:
            print(f"[ROUTER WARNING] Unknown agent '{agent}' returned. Falling back to CONVERSATION_AGENT.")
            agent = "CONVERSATION_AGENT"

        print(f"Final routed agent: {agent}")

        # Update state with SAFE agent only
        updated_state = {
            **state,
            "agent_name": agent,
        }
    ...
    def run_plant_disease_agent(state: AgentState) -> AgentState:
        """Handle plant disease analysis."""
        print(f"Selected agent: PLANT_DISEASE_AGENT")
        current_input = state["current_input"]
        image_path = current_input.get("image", None)

        if image_path:
            # Call the lazy-loaded agent
            result = plant_agent.analyze(image_path)
            
            if "error" in result:
                content = f"I encountered an error analyzing the plant image: {result['error']}"
            else:
                content = f"**Diagnosis:** {result['diagnosis']}\n**Confidence:** {result.get('details', '')}\n\nI recommend verifying this with a local agronomist."
        else:
            content = "No image provided for plant diagnosis."

        return {
            **state,
            "output": AIMessage(content=content),
            "needs_human_validation": True, # Critical info needs check
            "agent_name": "PLANT_DISEASE_AGENT"
        }
    
    def run_pest_agent(state: AgentState) -> AgentState:
        """Handle insect/pest analysis."""
        print(f"Selected agent: INSECT_AGENT")
        current_input = state["current_input"]
        image_path = current_input.get("image", None)

        if image_path:
            result = pest_agent.analyze(image_path)
            print(result)
            if "error" in result:
                content = f"Error identifying pest: {result['error']}"
            else:
                # Handle HuggingFace image-classification output
                label = None
                confidence = None

                if isinstance(result, list) and len(result) > 0:
                    label = result[0].get("pest_type")
                    confidence = result[0].get("confidence")
                elif isinstance(result, dict):
                    label = result.get("pest_type")
                    confidence = result.get("confidence")
            
            print(f"Identified pest: {label} with confidence {confidence}")

            if label:
                content = (
                    f"**Identified Pest:** {label}\n"
                    f"**Confidence:** {round(confidence * 100, 2) if confidence else 'N/A'}%\n\n"
                    "This is an AI-based identification. Please confirm with a local agriculture officer if needed."
                )
            else:
                content = "I could not confidently identify the insect from this image."

        else:
            content = "No image provided for pest identification."
        return {
            **state,
            "output": AIMessage(content=content),
            "needs_human_validation": True,
            "agent_name": "INSECT_AGENT"
        }
    ...
    #         **state,
    #         "output": response,
    #         "needs_human_validation": False,  # Medical diagnosis always needs validation
    #         "agent_name": "GeneralImageAgent"
    #     }
    def run_general_agent(state: AgentState) -> AgentState:
        print(f"Selected agent: GENERAL_AGENT")
        current_input = state["current_input"]
        image_path = current_input.get("image", None)
        
        # Extract text query or use default
        text_query = "What is in this image?"
        if isinstance(current_input, dict):
            text_query = current_input.get("text", text_query)

        if image_path:
            # Call Gemini Flash
            result = general_agent.analyze(image_path, user_query=text_query)
            if "error" in result:
                content = f"Error: {result['error']}"
            else:
                content = f"**Visual Analysis:** {result['answer']}"
        else:
        ...
        return {
            **state,
            "output": AIMessage(content=content),
            "needs_human_validation": False, # General queries are safer
    ...
    def perform_human_validation(state: AgentState) -> AgentState:
        ...
        validation_prompt = f"{state['output'].content}\n\n**Human Validation Required:**\n- If you are an Agricultural Expert: Please validate the diagnosis. Select **Yes** or **No**.\n- If you are a Farmer: Click Yes to acknowledge the automated advice."
        ...
        if "Human Validation Required" in output_text:
            ...
            if validation_input.lower().startswith(('yes', 'no')):
                ...
                if validation_input.lower().startswith('no'):
                    fallback_message = AIMessage(content="The automated analysis has been flagged by the expert as potentially inaccurate. Please consult a physical extension officer.")
                    return {
                        **state,
    ...
    workflow.add_node("PLANT_DISEASE_AGENT", run_plant_disease_agent)
    workflow.add_node("INSECT_AGENT", run_pest_agent)
    ...
    workflow.add_conditional_edges(
        ...
        lambda x: x["next"],
        {
            ...
            "PLANT_DISEASE_AGENT": "PLANT_DISEASE_AGENT",
    ...
    # Vision agents go to validation
    workflow.add_edge("PLANT_DISEASE_AGENT", "check_validation")
    workflow.add_edge("INSECT_AGENT", "check_validation")
    workflow.add_edge("GENERAL_AGENT", "check_validation")
